...
import { split, mix, generateBagOfChars, shuffleArray } from '../word-generation.ts';
...
describe('word-generation', () => {
  ...
  describe('mix', () => {
    ...
    it('should add spaces based on selection length', () => {
      ...
      const expectedSpaces = Math.floor(
        (splitWord.length + alphabet.length + splitWord.length * 3) / 3
      );
  ...
  describe('generateBagOfChars', () => {
    ...
    describe('FR-002: Only include remaining letters', () => {
      ...
      it('should include random letters for variance', () => {
        ...
        const randomLetters = result.filter(
          (c) => c !== 'a' && c !== 'b' && c !== 'c' && c !== ' '
        );
