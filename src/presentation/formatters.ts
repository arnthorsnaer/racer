 ...
 */
export const createHorizontalBorder = (
  width: number,
  leftPadding: number,
  isTop: boolean
): string => {
...
export const buildGameScreen = (
  ...
  lines.push(
    `${colors.arcadeGreen}F1: Hljóð  ^C: Hætta${colors.reset}  ${colors.sunsetOrange}• Ýttu á ▶${colors.reset}`
  );
...
export const buildWelcomeScreen = (
  ...
  lines.push('');
  lines.push(
    `${colors.bright}${colors.electricBlue}Stig ${currentLevel} (${targetWord.length}-stafa orð)${colors.reset}`
  );
  lines.push(`${colors.bright}Markmiðsorð: ${colors.sunsetOrange}${targetWord}${colors.reset}`);
  lines.push(
    `${colors.limeGreen}Orð kláruð: ${completedWords}${colors.reset}  ${colors.cosmicPurple}Stig: ${score.toFixed(1)}${colors.reset}`
  );
  lines.push('');
  lines.push(`${colors.electricCyan}Stjórnun:${colors.reset}`);
  lines.push(
    `  ${colors.arcadeGreen}▶ Stafir birtast sjálfkrafa á 0.6 sekúndu fresti${colors.reset}`
  );
...
export const buildCompletionScreen = (
  ...
  lines.push('');
  lines.push(
    `${colors.bright}${colors.neonPink}★ TIL HAMINGJU! Þú kláraðir orðið: ${colors.sunsetOrange}${fullTarget}${colors.reset}`
  );
  ...
  if (stats.isPerfect) {
    ...
    lines.push(
      `${colors.neonPink}Villur (rangar lyklar): ${colors.hotPink}${stats.errorCount}${colors.reset}`
    );
    lines.push(
      `${colors.neonPink}Missaðir stafir: ${colors.hotPink}${stats.missedLetters}${colors.reset}`
    );
    lines.push('');
...
export const buildNextLevelScreen = (
  ...
  lines.push('');
  lines.push(
    `${colors.bright}${colors.electricBlue}Borð ${currentLevel + 1}/${totalLevels}${colors.reset}`
  );
  lines.push(`${colors.bright}Markmiðsorð: ${colors.sunsetOrange}${targetWord}${colors.reset}`);
  lines.push('');
  lines.push(`${colors.limeGreen}Leikur byrjar...${colors.reset}`);
...
export const renderWithFrame = (
  ...
  const { leftPadding, topPadding } = getCenteringInfo(
    frameWidth + 2,
    contentHeight,
    termWidth,
    termHeight
